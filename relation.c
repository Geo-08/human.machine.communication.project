...
/*int call_radix(relation *rel){
	
	rel2 = (relation*)malloc(sizeof (relation));
	rel2->tuples = (tuple*)malloc(sizeof(tuple)*rel->num_tuples);
	rel->num_tuples = rel->num_tuples;

}

int radix_sort(relation *rel,relation *rel2, int depth){	//call it with depth argument value 0, rel will point at a sorted relation at the end.
	uint64_t hist[256];
	create_hist(rel,hist,depth);
	uint64_t psum[256]; 
	create_psum(hist,psum);
...
int create_psum (uint64_t *hist,uint64_t* psum){ //creates psum
	//int psum[256];
	int i;
	psum[0] = 0;
	for (i = 1; i<256;i++)
